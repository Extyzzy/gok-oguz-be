...
export class NonWorkingDaysService {
  ...
  async createDateByDate(date: string, num: number) {
    console.log('non-working-days.service.ts - createDateByDate()...');
    console.log(
      'non-working-days.service.ts - createDateByDate() - date: ',
      date,
      ', num: ',
      num,
    );
    ...
    console.log(
      'non-working-days.service.ts - createDateByDate() - timestamp: ',
      timestamp,
    );
  ...
  async getBeforeDate(date: string) {
    console.log(
      'non-working-days.service.ts - getRecgetRecBeforeDateDate() - date: ',
      date,
    );

    const timestamp = new Date(date);
    console.log(
      'non-working-days.service.ts - getRecgetRecBeforeDateDate() - timestamp: ',
      timestamp,
    );

    const ret = await this.RepositoryNonWorkingDays.find({
      where: { firstDay: LessThan(timestamp) },
    });
    console.log(
      'non-working-days.service.ts - getRecgetRecBeforeDateDate() - ret: ',
      ret,
    );
    if (!ret) {
      console.log(
        `non-working-days.service.ts - getRecgetRecBeforeDateDate() - record not found (date: ${date})`,
      );
  ...
  async updateRecordByDate(
    ...
    console.log(
      `non-working-days.service.ts - updateRecordByDate() - date: ${date}, newDate: ${newDate}, numberOfDays: ${numberOfDays}`,
    );
    ...
    const record = await this.RepositoryNonWorkingDays.findOne({
      where: { firstDay: dateTimestamp },
    });
    if (!record) {
      console.log(
        `non-working-days.service.ts - updateRecordByDate() - record not found (date: ${date})`,
      );
      return `record not found (date: ${date})`;
    }
  ...
  async updateRecordById(id: number, newDate: string, numberOfDays: number) {
    console.log(
      `non-working-days.service.ts - updateRecordById() - id: ${id}, newDate: ${newDate}, numberOfDays: ${numberOfDays}`,
    );
    ...
    if (!record) {
      console.log(
        `non-working-days.service.ts - updateRecordById() - record not found (id: ${id})`,
      );
